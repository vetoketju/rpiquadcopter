...
void  readGyro(double r[4])
{
        ...
        //0 on x, 1 on y, 2 on z, 3 on lämpötila
...
int main ()
{
        ...
        if(!initGyro()){
                printf("Gyron init epäonnistui, tarkista gpio load i2c");
                return -1;

        }

        if(!initAccelerometer()){
                printf("Accelerometerin init epäonnistui");
                return -1;
        }

        if(!initMagnetometer()){
              printf("Magnetometer init epäonnistui");
                return -1;
        }
